...
class AgentDQN(Agent):
    ...
    def next(self, state, turn,greedy_strategy):
        ...
        state_rep = self.state_to_representation_last(state=state) # sequence representation the state.

        if greedy_strategy == 1:
            greedy = random.random()
            if greedy < self.parameter.get("epsilon"):
                action_index = random.randint(0, len(self.action_sapce) - 1) # random select a action
            else:
